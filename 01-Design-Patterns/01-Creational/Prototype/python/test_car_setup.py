import pytest
import copy
from car_setup import CarSetup, EngineConfiguration
...
def test_cloning_creates_new_object_reference():
    ...
    original_setup = CarSetup(front_wing_angle=4, engine=engine)
...
def test_deep_copy_independence():
    ...
    setup_hamilton = CarSetup(front_wing_angle=6, engine=engine_ham)
    
    # 2. Clone for Teammate (Russell's Car)
    setup_russell = setup_hamilton.clone()
    
    # 3. Modify Russell's Engine
    setup_russell.engine.mode = "Save_Fuel"
    ...
    assert setup_russell.engine.mode == "Save_Fuel"
...
def test_independent_primitive_modification():
    ...
    setup_a = CarSetup(front_wing_angle=5, engine=engine)
